...
std::vector<torch::Tensor> hybrid_model_forward_cuda(
    ...
    const long batch_size = input.size(0);
    const long num_gaussians = gaussian_means.size(0);
    const long num_waves = wave_means.size(0);
    const long output_size = gaussian_colors.size(1);
    ...
    const int blocks = numSMs*64;

    // Dispatch jobs
    if(num_gaussians > 0){
        AT_DISPATCH_FLOATING_TYPES(input.type(), "gaussian_cuda_forward", ([&] {
        gaussian_forward_cuda_kernel<scalar_t><<<blocks, threads>>>(
    ...
    if(num_waves > 0){
        AT_DISPATCH_FLOATING_TYPES(input.type(), "wave_cuda_forward", ([&] {
        wave_forward_cuda_kernel<scalar_t><<<blocks, threads>>>(
            input.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
...
std::vector<torch::Tensor> hybrid_model_backward_cuda(
    ...
    torch::Tensor wave_coefficients     // [W, 5]
        ...
        if(num_gaussians > 0){
            ...
            gaussian_backward_cuda_kernel<scalar_t><<<blocks, threads>>>(
        ...
        if(num_waves > 0){
            AT_DISPATCH_FLOATING_TYPES(input.type(), "wave_cuda_backward", ([&] {
            wave_backward_cuda_kernel<scalar_t><<<blocks, threads>>>(
                grad_output.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
                input.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
                wave_colors.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
                wave_means.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
