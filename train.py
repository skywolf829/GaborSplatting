...
if __name__ == '__main__':
    ...
    model = GaussianSplatting2D(1000, device=device)
    ...
    for i in t:
        ...
        optim.step()
        with torch.no_grad():
            p = psnr(model_out, training_img_colors)
        ...
        t.set_description(f"[{i+1}/{n_iters}] loss: {loss.item():0.04f}, PSNR: {p:0.02f}")
    time_end = time.time()
    print(f"Final PSNR: {p:0.02f}")
    print(f"Elapsed time: {time_end-time_start:0.02f} seconds.")
