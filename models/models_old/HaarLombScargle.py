...
class HaarLombScargleModel():
    ...
    def generate_waves(self, v):
        return torch.stack([torch.sin(v),
                            torch.cos(v),
                            torch.sign(torch.sin(v)), 
                            torch.sign(torch.cos(v)), 
                            torch.ones_like(v)], dim=-1)
    ...
    def fit_angle_perfect(self, angles, idx, chi2, yw):
        ...
        self.wave_coefficients[idx] = theta_MLE.squeeze().reshape(-1, 5)
        self.power[idx] = (XTy.reshape(-1, 5) @ theta_MLE.reshape(-1, 5).t()).sum(dim=-1) / chi2
    ...
    def fit(self, frequencies:torch.Tensor, angles: List[torch.Tensor] = None, linear_in_frequency : bool=False, perfect_fit: bool = False):
        ...
        self.wave_coefficients = torch.empty([*angles_shapes, frequencies.shape[0], 5],
                                              device=self.device, dtype=torch.float32)
